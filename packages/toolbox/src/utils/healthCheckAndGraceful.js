...
import { mongoCloseConnection } from '../mongoose.js';
...
export function healthCheckAndGraceful(server) {
  ...
  const cleanUp = async () => {
    try {
      ...
      if (config.get('consul.enable')) promises.push(consul.close());

      if (config.get('mongoDb.enable')) promises.push(mongoCloseConnection());

      if (config.get('postgresDb.enable')) promises.push(pgPool.end());

      if (config.get('trace.enable')) promises.push(tracer.close());
  ...
  const beforeShutdown = () => {
    logger.info(`${serviceName} started graceful shutdown, waiting for requests to drain ${delay} milSeconds`, logMetaData);
