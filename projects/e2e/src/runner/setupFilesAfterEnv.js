...
jasmine.getEnv().addReporter({
  async specDone({ status, description, fullName, failedExpectations }) {
    try {
      ...
      if (status === 'pending')
        process.stdout.write(`${suiteName.padEnd(45)} | ${description.padEnd(97)} | ${chalk.bgBlue.black.bold('skipped')}\n`);

      if (status === 'passed')
        process.stdout.write(`${suiteName.padEnd(45)} | ${description.padEnd(97)} | ${chalk.bgGreen.black.bold('passed')}\n`);
